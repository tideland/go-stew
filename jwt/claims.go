...
func (c Claims) GetMarshalled(key string, v any) (bool, error) {
	value, ok := c.Get(key)
	if !ok {
		return false, nil
	}
...
func (c Claims) Audience() ([]string, bool) {
	...
	makeStrings := func(auds ...any) ([]string, bool) {
		if len(auds) == 0 {
			return nil, false
		}
...
func (c *Claims) UnmarshalJSON(b []byte) error {
	...
	if err := json.Unmarshal(b, &raw); err != nil {
		return fmt.Errorf("error unmarshalling claims from JSON: %v", err)
	}
	*c = Claims(raw)
	return nil
}
