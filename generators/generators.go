...
// SentenceWithNames works like Sentence but inserts randomly
// names of the passed set. They can be generated by NameSet().
func (g *Generator) SentenceWithNames(names []string) string {
	count := g.Int(2, 15)
	words := []string{}
	for _, word := range g.Words(count) {
		if g.FlipCoin(90) {
			words = append(words, g.OneStringOf(names...))
		}
...
// ParagraphWithNames workes like Paragraph but inserts randomly
// names of the passed set. They can be generated by NameSet().
...
var words = []string{
	...
	"facer", "facili", "facilisis", "fames", "faucibus", "felis", "fermentum",
	...
	"rebum", "rhoncus", "riduculus", "risus", "rutrum",
